...
![Image text](https://github.com/chaopan95/JS-offer/blob/main/_img/1.png)
...
public：
    void reOrderArray(vector<int> &array) {
        ...
        for (int i = 0, j = 0; i < int(odd.size()); i++, j++)
        {
            array[j] = odd[i];
        }
        for (int i = 0, j = int(odd.size()); i < int(even.size());
             i++, j++)
        {
            array[j] = even[i];
        }
...
public：
    ListNode* ReverseList(ListNode* pHead) {
        ...
        ListNode *p1 = NULL, *p2 = pHead, *p3 = pHead->next;
        while (p3 != NULL)
        {
            p2->next = p1;
            p1 = p2;
...
public：
    ListNode* Merge(ListNode* pHead1, ListNode* pHead2)
    {
        ...
        ListNode *cur = head, *p1 = pHead1, *p2 = pHead2;
...
public：
    bool HasSubtree(TreeNode* pRoot1, TreeNode* pRoot2)
    {
        ...
        qTree.push(pRoot1);
        TreeNode *front = pRoot1;
        while (!qTree.empty())
        {
            front = qTree.front();
            qTree.pop();
            if (front->val == pRoot2->val &&
                isSame(front, pRoot2)) { return true; }
            if (front->left != NULL) { qTree.push(front->left); }
            if (front->right != NULL) { qTree.push(front->right); }
        }
        return false;
    }
...
public：
    vector<int> printMatrix(vector<vector<int> > matrix) {
        ...
        bool **dp = new bool *[nRows];
        for (int i = 0; i < nRows; i++) { dp[i] = new bool [nCols]{}; }
        int i = 0, j = 0;
        ...
        while (true)
        {
            ...
            for (int k = 0; k < 4; k++)
            {
                while (isOk(dp, i+di[k], j+dj[k], nRows, nCols))
                {
        ...
        for (i = 0; i < nRows; i++) { delete []dp[i]; }
        delete []dp;
        return res;
    }
    bool isOk(bool **isVisited, int i, int j, int nRows, int nCols)
    {
        if (i < 0 || i >= nRows || j < 0 || j >= nCols ||
            isVisited[i][j]) { return false; }
        return true;
    }
};
```
...
public：
    vector<vector<int> > FindPath(TreeNode* root,int expectNumber) {
        ...
        if (root == NULL) { return res; }
        vector<int> arr;
        path(root, expectNumber, res, arr, 0);
        return res;
    }
              ...
              vector<int> &arr, int sum)
    {
        ...
        {
            path(root->left, target, res, arr, sum+root->val);
            path(root->right, target, res, arr, sum+root->val);
        }
...
public：
    ...
    int MoreThanHalfNum_Solution(vector<int> numbers) {
        ...
        if (n % 2 && numbers[n/2] != numbers[n/2-1]) { return 0; }
        int res = numbers[n/2];
        int count = 1;
        int pos = n/2 - 1;
        while (pos >= 0)
        {
            if (numbers[pos] != res) { break; }
            pos--;
            count++;
        }
        pos = n/2 + 1;
...
Solution：Take 2593 as an example, how many '5' does it contain?

i = 1, Quotient = 2593 / (10 * i) = 258, Mod = 2593 % (10 * i) = 3. Number of '5'： 259 * i + (3 > 5) * i = 259.

i = 10, Quotient = 2593 / (10 * i) = 25, Mod = 2593 % (10 * i) = 93. Number of '5'： 25 * i + (9 > 5) * i = 260.

i = 100, Quotient = 2593 / (10 * i) = 2, Mod = 2593 % (10 * i) = 593. Number of '5'： 2 * i + (5 == 5) * (93 + 1) = 294.

i = 1000, Quotient = 2593 / (10 * i) = 0 end.Total number of '5'： 259 + 260 + 294 = 813.
...
public：
    ...
    {
        int res = 0;
        for (int i = 1; i <= n; i++) { res += count1(i); }
        return res;
    }
    int count1(int num)
    {
...
public：
    int NumberOf1Between1AndN_Solution(int n)
    {
        ...
        {
            int a = n / (10 * i), b = n % (10 * i);
            count += a * i;
            if (b / i > digit) { count += i; }
            if (b / i == digit) { count += b % i + 1; }
        }
        return count;
    }
};
